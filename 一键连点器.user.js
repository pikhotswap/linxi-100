// ==UserScript==
// @name         一键连点器（结界 / 砍树 / 强化 / 配装）
// @namespace    lottery-multi-click
// @version      1.0.0
// @description  一键自动点击按钮 N 次：结界连点、连砍 N 刀、连续强化（自动关结果框）、仲裁攻击；灵台配装分析与一键换装。
// @match        https://test-sub2api.shengqainbang.cn/*
// @match        https://k40.shengqainbang.cn/*
// @match        https://sub.100xlabs.space/*
...
(() => {
  // 防重复注入：清掉旧的面板/卡片（例如 Chrome 扩展版还在运行时）
  for (const id of ['mc-panel', 'mc-card']) {
  ...
  // 仲裁按钮：仲裁页签里的 lt-btn raid，文字「攻击」。
  // 只在仲裁页签激活时才渲染，其他页签下找不到就提示用户先切页签。
  function findRaidBtn() {
    const btn = document.querySelector('button.lt-btn.raid');
    if (btn && btn.offsetParent) return btn;
    for (const el of document.querySelectorAll('button')) {
      if (!el.offsetParent) continue;
      if ((el.innerText || '').trim() === '攻击') return el;
    }
    return null;
  }
  ...
  function findTargetBtn(mode) {
    ...
    if (mode === 'raid') return findRaidBtn();
    return mode === 'enhance' ? findEnhanceBtn() : findBatchChopBtn();
  }

  // ================= 弹窗处理（强化结果框）=================
  // 注意：.modal-overlay 的 CSS 是 position:fixed，而 fixed 元素的 offsetParent
  // 按规范恒为 null。之前用 offsetParent 判可见性，findModal() 永远返回 null，
  // 于是「等弹窗」每次都超时、closeModal 从来没被调用过——框就留在那儿等人手点。
  // 改成按几何尺寸判：卸载 / display:none / 零尺寸都会让 rect 全是 0。
  ...
  function findModal() {
    const list = document.querySelectorAll('div.modal-overlay');
    for (let i = list.length - 1; i >= 0; i--) {
      if (isVisible(list[i])) return list[i]; // 后出现的优先
    }
    return null;
  }
  ...
  // 关框有三条路，挨个试并验证：
  //   ① 右上角 X   ② Esc   ③ 点遮罩
  // 绝不点 footer 按钮——「分解装备」用的是同一套 BaseDialog 组件，它的 footer 是
  // 「确认分解」，盲点会把装备分解掉。
  // 遮罩点击只在 closeOnClickOutside 为真时才生效（BaseDialog 默认 false），
  // 所以对分解框是空操作，安全。
  async function dismissModal() {
    const opens = () => {
      const m = findModal();
      if (!m) return null;
      const x = m.querySelector('button[aria-label="Close modal"]');
      if (x && !x.disabled) return x;
      return null;
    };
    for (let round = 0; round < 2; round++) {
      ...
      if (await waitModalGone(800)) return true;
    }
    return !findModal();
  }
  ...
  goBtn.addEventListener('click', async () => {
    ...
    if (!first) {
      say(mode === 'enhance' ? '❗ 没找到强化按钮（先选中一件装备）'
        : mode === 'raid' ? '❗ 没找到仲裁按钮（先切到「仲裁」页签）'
        : '❗ 没找到目标按钮');
      return;
    }
    ...
    for (let i = 1; i <= n; i++) {
      ...
      if (btn.disabled) {
        ...
        if (!autoClicking) break;
        if (!alive) {
          // 体力耗尽 / 灵木不足 / 已满级 / 仲裁次数或免费额度用完，按钮不会再恢复
          say(`按钮不可用，已停止（${ok}/${n}）`);
          break;
        }
      }

      // 上一轮的框要是没关掉（网络慢、弹窗晚到），先清掉再点下一次，
      // 否则框会越积越多、按钮也可能被挡住。
      if (mode === 'enhance' && findModal()) await dismissModal();
      ...
      if (mode === 'enhance') {
        ...
        if (modal) {
          const closed = await dismissModal();
          if (!closed) {
            say(`❗ 强化结果框关不掉，已停止（${ok}/${n}）——请手动关掉后重试`);
            break;
          }
        }
      } else if (mode === 'raid') {
    ...
    if (finished) {
      ...
      else if (mode === 'raid') say(`完成 ✓ 仲裁攻击 ${ok} 次`);
